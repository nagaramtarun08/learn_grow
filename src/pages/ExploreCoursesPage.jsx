...
import { Filter, Search, Star, Users, ExternalLink, FileText } from 'lucide-react';
...
const ExploreCoursesPage = () => {
  ...
  useEffect(() => {
    // Fetch trainer-created courses from localStorage
    const trainerCourses = [];
    Object.keys(localStorage).forEach((key) => {
      if (key.startsWith('trainerCourses_')) {
        const userCourses = JSON.parse(localStorage.getItem(key)) || [];
        trainerCourses.push(...userCourses);
      }
    });

    // Combine sample courses and trainer-created courses
    const allCourses = [...sampleCourses, ...trainerCourses];

    // Fetch associated content for each course
    const allContent = [];
    Object.keys(localStorage).forEach((key) => {
      if (key.startsWith('trainerContent_')) {
        const userContent = JSON.parse(localStorage.getItem(key)) || [];
        allContent.push(...userContent);
      }
    });

    // Attach content to respective courses
    const coursesWithContent = allCourses.map((course) => ({
      ...course,
      content: allContent.filter((content) => content.courseId == course.id),
    }));
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredCourses.map((course, index) => (
              <motion.div
                ...
                className="flex"
              >
                <Card className="w-sort flex flex-col overflow-hidden shadow-lg hover:shadow-2xl transition-shadow duration-300 glassmorphism transform hover:-translate-y-1">
                  ...
                  <CardContent className="flex-grow">
                    ...
                    {course.content && course.content.length > 0 && (
                      <div className="mt-3">
                        <p className="text-sm font-medium text-gray-700 dark:text-gray-300">Available Content:</p>
                        <ul className="text-sm text-gray-600 dark:text-gray-400">
                          {course.content.map((item) => (
                            <li key={item.id} className="flex items-center">
                              <FileText className="h-4 w-4 mr-1" /> {item.fileType}: <a href={item.fileUrl} target="_blank" rel="noopener noreferrer" className="underline">{item.fileUrl}</a>
                            </li>
                          ))}
                        </ul>
                      </div>
